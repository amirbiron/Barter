...
const reporter = new ActivityReporter(
    ...
    process.env.ACTIVITY_SERVICE_ID || 'srv-d2mf9m3uibrs73bbkhsg',
    process.env.ACTIVITY_SERVICE_NAME || 'Barter'
);
...
function isMaintenanceMode() {
    ...
    return (
        ...
        process.uptime() * 1000 < STARTUP_MAINTENANCE_WINDOW_MS
    );
}
...
function getPostActionsKeyboard(postId, userId) {
    const isAdmin = config.isAdmin(userId);
    return keyboards.getPostActionsKeyboardForAdmin(postId, isAdmin);
}
...
bot.on('message', async (msg) => {
    ...
    try {
        ...
        console.log(
            `🔍 השוואת כפתור חיפוש: "${text}" === "${searchButtonText}" ? ${text === searchButtonText}`
        );
        ...
        switch (text) {
            ...
            case '📌 חיפוש בכותרות בלבד':
                if (userState.step === 'search_type') {
                    ...
                    await bot.sendMessage(
                        chatId,
                        config.messages.unknownCommand,
                        getMainKeyboard()
                    );
                }
                break;
            ...
            case '❌ ביטול':
                ...
                if (
                    userState.step === 'search' ||
                    userState.step === 'search_type' ||
                    userState.step === 'search_titles' ||
                    userState.step === 'search_full'
                ) {
    ...
    switch (userState.step) {
        case 'title':
            ...
            if (text.length > config.content.maxTitleLength) {
                await bot.sendMessage(
                    chatId,
                    `❌ כותרת ארוכה מדי (מקסימום ${config.content.maxTitleLength} תווים). נסו שוב:`
                );
                return;
            }
        ...
        case 'description':
            ...
            if (text.length > config.content.maxDescriptionLength) {
                await bot.sendMessage(
                    chatId,
                    `❌ תיאור ארוך מדי (מקסימום ${config.content.maxDescriptionLength} תווים). נסו שוב:`
                );
                return;
            }
            ...
            setUserState(userId, {
                ...userState,
                step: 'pricing',
                description: utils.sanitizeText(text),
            });
        ...
        case 'price_range': {
            ...
            setUserState(userId, {
                ...userState,
                step: 'portfolio',
                price_range: priceValidation.formatted,
            });
            await bot.sendMessage(
                chatId,
        ...
        case 'contact': {
            ...
            setUserState(userId, {
                ...userState,
                step: 'tags',
                contact_info: contactValidation.formatted,
            });
            await bot.sendMessage(
                chatId,
        ...
        case 'tags': {
            ...
            if (config.isAdmin(userId)) {
                ...
                await bot.sendMessage(
                    chatId,
                    '🔐 האם המודעה תהיה ציבורית או פרטית?\n\n' +
...
async function handleSearch(chatId, query, userId) {
    ...
    try {
        ...
        console.log(
            `📤 מציג ${Math.min(results.length, maxResults)} תוצאות מתוך ${results.length}`
        );
        ...
        for (const post of results.slice(0, maxResults)) {
            ...
            await bot.sendMessage(chatId, message, {
                parse_mode: 'Markdown',
                ...getPostActionsKeyboard(post.id, userId),
            });
        ...
        if (results.length > maxResults) {
            await bot.sendMessage(
                chatId,
                `📄 יש עוד ${results.length - maxResults} תוצאות. חדדו את החיפוש לתוצאות טובות יותר.`
            );
        }
...
async function handleTitleSearch(chatId, query) {
    ...
    try {
        ...
        const message =
            `📌 נמצאו ${results.length} תוצאות לחיפוש "${query}":\n\n` +
                ...
                if (post && post.is_active) {
                    ...
                    if (isAdmin) {
                        inline.push([
                            ...
                            {
                                text: `${e ? '🔙 ' : ''}חזרה לרשימה`,
                                callback_data: `browse_${browseType}_page_${page}`,
                            },
                        ]);
                    } else {
                        inline.push([
                ...
                reply_markup: {
                    inline_keyboard: [
                        [
                            {
                                text: '✅ אשר מחיקה',
                                callback_data: `confirm_admin_delete_${postId}`,
                            },
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                                    text: '👁️ צפה במודעה',
                                    callback_data: `view_post_${postId}_from_alert`,
                                },
...
async function handlePricingSelection(chatId, userId, data) {
    ...
    if (pricingMode === 'payment' || pricingMode === 'both') {
        await bot.sendMessage(
            chatId,
            '💵 הקלידו את טווח המחיר (דוגמא: "100-500 ש״ח" או הקלידו "דלג"):'
        );
    ...
    try {
        ...
        currentPosts.forEach((post, index) => {
            ...
            if (
                ...
                (post.pricing_mode === 'both' &&
                    post.price_range &&
                    post.price_range.includes('חינם'))
            ) {
        ...
        if (page > 1) {
            navRow.push({
                text: e ? '◀️ הקודם' : '< הקודם',
                callback_data: `browse_${browseType}_page_${page - 1}`,
            });
        }
        ...
        keyboard.push([
            {
                text: e ? '🔙 חזרה לאפשרויות דפדוף' : 'חזרה לאפשרויות דפדוף',
                callback_data: 'back_to_browse_options',
            },
        ]);
...
async function handleAddKeyword(chatId, userId) {
    ...
    await bot.sendMessage(
        ...
        '✏️ *הוספת מילות מפתח*\n\n' +
            '💬 אפשר להקליד:\n' +
            '• מילה בודדת (למשל: `עיצוב`)\n' +
            '• ביטוי מלא (למשל: `עיצוב גרפי`)\n' +
            '• כמה מילים/ביטויים מופרדים בפסיק\n\n' +
            '📌 *דוגמה לרשימה מלאה:*\n' +
            '`עיצוב, עיצוב גרפי, פוטושופ, פיתוח אתרים, React`\n\n' +
            '⚡ *חשוב:* השתמש בפסיק (,) להפרדה בין מילים\n\n' +
            '_הקלד את המילים שלך ושלח:_',
        {
            parse_mode: 'Markdown',
